    ...
    return {
        ...
        execute: (function () {
            ...
            const meta = exports('meta', metaBeauty({
                ...
                author: 'eczn',
                intro: '前段时间工作重心在性能优化，着重看了 V8 相关的优化细节，尤其是 Inline Caches (ICs) 技术，后面依据这项技术将核心指标性能优化到原来的十倍，这里记录一下 ICs 相关细节，仅供参考: 解释器手写的 line-by-line 虚拟机执行流水线完全比不上 CPU 内部的指令流水线以及更为夸张的分支预测手段，因此我个人过去认为 C 至少比 JS 快 10 倍+，直到最近看了不少的 v8 benchmark 之后才对这块有所改观，目前的观点是：只要构造合适，V8 JIT 可以有媲美原生 AOT 的性能，当然也容易出现非常夸张的性能劣化。',
                // category: '分类',
