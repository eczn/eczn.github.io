...
function jPenCC(jP){
	...
	function circleM(){
		...
		g.beginPath();
		g.fillStyle = 'rgba(183,183,183,0.3)';
		g.fillRect(0,0,width,height);
		g.closePath(); 
...
function toPlay() {
	...
	document.removeEventListener('click', toPlay);
	document.removeEventListener('touchstart', toPlay);

}

document.addEventListener('click', toPlay);
document.addEventListener('touchstart', toPlay);
